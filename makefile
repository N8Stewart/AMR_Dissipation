...
pd: $(COMMON_FILES) $(PTHREAD_DISPOSABLE_FILE)
	gcc $(COMMON_ARGUMENTS) $(THREAD_ARGUMENTS) $(PTHREAD_DISPOSABLE_FILE) -o $(PTHREAD_DISPOSABLE_OUTPUT)
# Pthread persistent threads. Lab2b
pp: $(COMMON_FILES) $(PTHREAD_PERSISTENT_FILE)
	gcc $(COMMON_ARGUMENTS) $(THREAD_ARGUMENTS) $(PTHREAD_PERSISTENT_FILE) -o $(PTHREAD_PERSISTENT_OUTPUT)
clean:
	rm -f $(BINARIES) *.out *.o  
